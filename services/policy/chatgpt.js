...
import { normalizeSpeciesProfile, } from '../../core/models/speciesProfile';
import { normalizePolicyPayload, PolicySchemaError, SUPPORTED_SPECIES_TYPES, } from '../../core/logic/policySchema';
...
const SEED_TYPES = ['succulent', 'semi-succulent', 'tropical', 'fern'];
const resolveSpeciesType = (type) => {
    if (!type)
        return 'other';
    const normalized = type.trim().toLowerCase();
    return SUPPORTED_SPECIES_TYPES.includes(normalized) ? normalized : 'other';
};
const sanitizeHint = (value) => value.trim();
...
const formatUserPrompt = (request, fallbackType) => {
...
const mapSeedPolicies = (overrides) => {
    const merged = { ...DEFAULT_POLICY_SEEDS };
    if (!overrides)
        return merged;
...
export class ChatGptPolicyService {
    ...
    async generate(request) {
        ...
        for (let attempt = 0; attempt < this.maxAttempts; attempt += 1) {
            ...
            if (attempt < this.maxAttempts - 1) {
                const guidance = lastError instanceof PolicySchemaError || lastError instanceof SyntaxError
                    ? lastError.message
    ...
    normalizeRequest(request) {
        ...
        return {
            ...
            type: request.type ? request.type.trim() : undefined,
    ...
    async invokeOpenAi(endpoint, apiKey, messages) {
        const headers = {
    ...
    buildProfile(request, normalized, source, timestamp) {
        const profile = {
    ...
    buildSeedProfile(request, fallbackType, timestamp) {
...
export const createChatGptPolicyService = (options = {}) => new ChatGptPolicyService(options);
export const getDefaultPolicySeeds = () => ({
