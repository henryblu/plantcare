...
import { createChatGptPolicyService } from './chatgpt';
describe('ChatGptPolicyService', () => {
    ...
    it('resolves relative proxy endpoint before invoking fetch', async () => {
        const fetchMock = vi.fn(async (input, init) => {
            ...
            return new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify(payload) } }] }), { headers: { 'Content-Type': 'application/json' } });
        });
