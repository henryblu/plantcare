...
const normalizeMockCandidates = (candidates) => candidates.map((candidate) => ({
    ...candidate,
    speciesKey: normalizeSpeciesKey(candidate.canonicalName, candidate.taxonId),
    source: 'mock',
}));
export const createManualCandidate = (input) => {
...
export const createIdentificationProvider = (options = {}) => {
    ...
    const identifyWithMocks = async (request) => {
    ...
    const identifyWithPlantNet = async (request) => {
