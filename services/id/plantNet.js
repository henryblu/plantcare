...
const toBlob = async (image, fetchFn) => {
...
const buildUrl = (options, request) => {
...
const normalizeCandidate = (result) => {
    if (!result.species)
        return null;
    const canonicalName = result.species.scientificNameWithoutAuthor || result.species.scientificName;
    if (!canonicalName)
        return null;
...
const sortAndLimit = (candidates, limit) => {
